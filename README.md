...
Projeto de exemplo que implementa um sistema em memória para cadastro de alunos, consulta de salas disponíveis, realização de reservas e consulta do histórico de reservas.
...
```powershell
# Testes com cobertura (mostra linhas não cobertas)
python -m pytest --cov=src --cov-report=term-missing

# Apenas testes (verbose)
python -m pytest -v

# Apenas testes (resumido)
python -m pytest -q
...
Se você estiver em um ambiente sem acesso à internet e sem `pytest`, pode usar a ferramenta interna fornecida neste projeto ou rodar manualmente os casos de teste apontados em `tests/`. O autor usou um mini-executor e o módulo `trace` para validação local antes da entrega.
